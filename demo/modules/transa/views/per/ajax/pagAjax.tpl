...
{foreach item=datos from=$per}
    ...
    <tr id="list" style="background:{$color}" onmouseover=style.background="#F0F8FF" onmouseout=style.background="{$color}">
        ...
        {if $_acl->permiso('elim_per')}<td style="text-align: center;"><a class="btn btn-small glyphicon glyphicon-trash" href="{$root}transa/per/delPer/{$datos.Id_encrypt}/{$datos.Cond_encrypt}" onclick='return cb(this);'></a></td>{/if}    
    </tr>
