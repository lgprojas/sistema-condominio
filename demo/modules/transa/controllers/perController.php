...
//        $this->_view->assign('trelv', $this->_per->getAllRelVehi());
    ...
    public function delPer($ide = false){
        ...
        if(!$this->_per->verificarPer($this->filtrarInt($id))){
            ...
            $idcond = $this->_per->getIdCondPer($this->filtrarInt($id));
        
        $this->_per->eliminarPer($this->filtrarInt($id), $idcond);
...
//                $this->_view->setJs(array('ajax_asoc'));
        ...
        if($this->getInt('guardar') == 1){
            ...
            if($isRel){
                $this->_view->assign('_error', 'La persona ya posee una relación de "'.$isRel["Nom_trel"].'" con la vivienda "'.$isRel["Nom_cb"].'-'.$isRel["Num_viv"].'".');
                $this->_view->renderizar('asocviv', 'per');
                exit;
            }
    ...
    public function asocVehiPer($ideper=false) {
        ...
        if(!Session::get('autenticado')){$this->redireccionar();}
        $this->_acl->acceso('asoc_vehi_per');
        ...
        if($this->getInt('guardar') == 1){
            ...
            $isRel = $this->_per->poseeYaRelVehiPer($this->getSql('vehi'),$idper,$this->getSql('trelv'));
            if($isRel){
                $this->_view->assign('_error', 'La relación "'.$isRel["Nom_trelv"].'" con vehículo ['.$isRel["Cod_vehi"].'] '.$isRel["Nom_marca"].' '.$isRel["Nom_modelo"].' ya se encuentra en la lista.');
                $this->_view->renderizar('asocvehi', 'per');
                exit;
            }
