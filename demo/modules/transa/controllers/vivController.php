...
class vivController extends transaController {
    ...
    public function newViv(){
        ...
        switch (Session::get('level')) {
            ...
            default:
                $co = $this->_viv->getVivIdCond(Session::get('id_usuario'));
                ...
                $allEstOk = $this->_viv->getEsttosOK($cond);
                $num = range(1, 600);
                foreach($num as $key => $valor){
                  if(array_search($valor, $allEstOk) !== false){
                       unset($num[$key]);
                  }
                }
                $this->_view->assign('estok', $num);
                //----------------------------------------------
        }
        ...
        if($this->getInt('guardar') == 1){
            ...
            $exiViv = $this->_viv->verificarExisteViv($this->getSql('num'),$this->getSql('cb'),$this->getSql('cond'));
            if($exiViv){
                $this->_view->assign('_error', 'La vivienda "'.$exiViv["Nom_cb"].'-'.$exiViv["Num_viv"].'" ya se encuestra registrada.');
                $this->_view->renderizar('editar', 'viv');
                exit;
            }
    ...
    public function getEstCond(){
        
        if($this->getInt('cond')){
          $cond = 'WHERE Id_cond = '. $this->getInt('cond');
          //filtra estacionamientos disponibles
                $allEstOk = $this->_viv->getEsttosOK($cond);
                $num = range(1, 600);
                $esta = array();
                for ($i = 1; $i < count($num); $i++) {
                  if(array_search($num[$i], $allEstOk) == false){
                       $esta[]= str_pad($num[$i],3,"0",STR_PAD_LEFT);
                  }
                  
                }
                //print_r($esta);
        }
        echo json_encode($esta);
    }
