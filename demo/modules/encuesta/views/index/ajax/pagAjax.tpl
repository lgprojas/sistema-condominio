...
<table class="table table-bordered">
    ...
    {foreach item=ca from=$encuestas}
        <tr>
            ...
            {if $ca.Id_estencu== 1}
                <a href="{$root}encuesta/index/editEstEncu/{$ca.Id_encrypt}" onclick='return en(this);'>
                    <img src="{$root}public/img/all/estrella_cpromo.png" width="15" height="15" data-toggle="tooltip" data-placement="top" title="Activada/desactivada para ser respondida."/>
                </a>
            {else}
                <a href="{$root}encuesta/index/editEstEncu/{$ca.Id_encrypt}" onclick='return en(this);'>
                    <img src="{$root}public/img/all/estrella_spromo.png" width="15" height="15" data-toggle="tooltip" data-placement="top" title="Activada/desactivada para ser respondida."/> 
                </a>
            {/if}
            </td>
            {/if}
            {if $_acl->permiso('ver_items')}<td style="text-align: center;"><a href="{$root}encuesta/index/addItems/{$ca.Id_encrypt}"><i data-toggle="tooltip" data-placement="top" title="Los ítems u opciones que tendrá la encuesta para ser respondida." class="glyphicon glyphicon-list-alt"></i></a></td>{/if}                       
            ...
            {if $_acl->permiso('ver_votos_encu')}<td style="text-align: center;"><a href="{$root}encuesta/index/estVotaEncu/{$ca.Id_encrypt}/{$ca.Cond_encrypt}" ><i data-toggle="tooltip" data-placement="top" title="Ver votación de la encuesta." class="glyphicon glyphicon-stats"></i></a></td>{/if}
            {if $_acl->permiso('elim_encu')}<td style="text-align: center;"><a href="{$root}encuesta/index/deleteEncu/{$ca.Id_encrypt}" onclick='return dc(this);'><i title="Eliminar" class="glyphicon glyphicon-trash"></i></a></td>{/if}            
        </tr>        
    {/foreach}
